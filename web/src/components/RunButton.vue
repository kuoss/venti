<template>
  <div class="relative" v-click-outside="close">
    <button
      ...
      @click="$emit('execute')"
    >
      <i class="mdi mdi-refresh" :class="{ 'mdi-spin': intervalSeconds > 0 }"></i>
      {{ btnText }}
    </button>
    ...
    <div class="absolute top-[100%] right-0 bg-white dark:bg-stone-900 border" v-if="isOpen">
      <div class="p-1 px-2 cursor-pointer hover:bg-gray-200" v-for="i in intervals" @click="selectInterval(i)">
        {{ i }}
      </div>
    </div>
  </div>
