...
<template>
  ...
  <main class="mt-12 w-full p-8 pb-16">
    <h1 class="py-2 font-bold">
      Alert Groups ({{ alertGroups.length }} groups)
    </h1>
    <table class="w-full bg-white border">
      ...
      <tbody v-for="g in alertGroups">
        <tr class="border-b">
          <th class="text-left bg-slate-200 p-3" colspan="6">
            {{ g.datasource == "Prometheus" ? "🔥" : "💧" }} {{ g.name }} ({{
              g.rules.length
            }}
            rules)
          </th>
        ...
        <tr class="border-b" v-for="r in g.rules">
          ...
          <td
            class="px-2 max-w-[20vw] truncate hover:whitespace-normal text-cyan-500"
          >
            <a
              class="hover:underline"
              :href="`/#/${
                g.datasource == 'Prometheus' ? 'metrics' : 'logs'
              }?query=${encodeURIComponent(r.expr)}`"
              >{{ r.expr }}</a
            >
          </td>
