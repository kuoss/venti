  ...
  <header class="fixed right-0 w-full bg-white border-b border-common shadow z-30 p-2 pl-52"
    ...
    <div class="flex items-center flex-row">
      ...
      <div class="flex ml-auto">
        ...
        <span class="ml-2">
          <RunButton btnText="Run query" :disabled="busy" @execute="execute" @changeInterval="changeInterval" />
        </span>
      </div>
    </div>
  </header>
            ...
            <table class="whitespace-nowrap border-separate w-full" style="border-spacing:0">
              ...
              <tr v-if="result && result.length > 0" v-for="row in result" class="border-b hover:bg-gray-200">
                <td
                  ...
                  v-for="key in keys" @mouseover="row.hover = row.hover || {}; row.hover[key] = true"
        ...
        <div>
          <ul class="w-full flex list-none">
            <li class="py-3 basis-1/2 text-center hover:bg-slate-50 cursor-pointer border-b-2 border-transparent"
              :class="tab == 0 ? 'active' : ''" @click="tab = 0">Metrics ({{ Object.keys(metadata).length }})</li>
            <li class="py-3 basis-1/2 text-center hover:bg-slate-50 cursor-pointer border-b-2 border-transparent"
              :class="tab == 1 ? 'active' : ''" @click="tab = 1">Labels ({{ keys.length }})</li>
          </ul>
        </div>
        <div
          ...
          <div v-if="tab == 0">
            <div class v-for="(d, k) in metaDict">
              <div class="pl-1 cursor-pointer text-stone-600" @click="d.showMetrics = !d.showMetrics">{{ k }} ({{
                  d.metrics.length
              }})</div>
              <div class="pl-4 overflow-hidden text-ellipsis hover:bg-white cursor-pointer" v-if="d.showMetrics"
                v-for="m in d.metrics" @click="applyMetric(m)" @mouseover="selectMetric(m)">{{ m.name }}</div>
            </div>
          </div>
          <div v-else>
